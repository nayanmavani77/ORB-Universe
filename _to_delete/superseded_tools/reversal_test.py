...
from orb.config import SL_FULL_RANGE                        # noqa: E402
from orb.engines.reversal import (ANCHOR_MIRROR, REVERSE,   # noqa: E402
                                  ReversalSettings)


# --------------------------------------------------------------------------
def reversal_settings_for(cfg, max_trades: int):
    """The reversal settings that reproduce this tool's original behaviour.

    This file used to carry its own `ReversedStrategy`, swapped in by rebinding
    `orb.engine.RangeBreakoutStrategy`. That class mirrored the ORIGINAL stop
    LEVEL around the entry, and the original level depended on `sl_mode` — the
    range midpoint (half the range) or the opposite side (the whole range).

    The reversal engine expresses exactly that as `sl_anchor: mirror` with the
    matching multiplier, so the old class is gone and this maps onto it. The
    numbers are unchanged; `tools/golden_master.py` case
    `reversal_LONDON_M5_SL0p5_RR2_RR_mirror` covers the mechanism.

    The swap had to go: `Engine` no longer reads that module global, so a
    monkey-patch would now do nothing at all — and a reversal study that
    silently executed forward trades would be worse than one that crashed.
    """
    session = next(iter(cfg.enabled_sessions()))
    mult = 1.0 if session.sl_mode == SL_FULL_RANGE else 0.5
    return ReversalSettings(sl_range_mult=mult, direction=REVERSE,
                            sl_anchor=ANCHOR_MIRROR,
                            max_trades_per_session=int(max_trades))
...
def run_variant(cfg: AppConfig, bars, reverse: bool, max_trades: int):
    ...
    if reverse:
        reversal_settings_for(run_cfg, max_trades).apply_to(run_cfg)
