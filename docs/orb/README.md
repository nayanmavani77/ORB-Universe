...
Config: `orb/engines/orb/config.yaml`  ·  Code: `orb/engines/orb/`  ·  Results: `backtest/orb/`
...
`OrbSettings` is therefore an empty settings class. It earns its place by
rejecting options that belong to another engine:

```yaml
engine: orb
engine_options:
  sl_range_mult: 0.75      # error at load — that is an orb_reverse option
```
