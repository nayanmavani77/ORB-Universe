...
Config: `orb/engines/orb_reverse/config.yaml`  ·  Code: `orb/engines/orb_reverse/`  ·  Results: `backtest/orb_reverse/`
...
|---|---|---|
| 0.25 | ¼ range | |
| **0.5** | ½ range | **identical to the `orb` engine's `mid_range`** |
| 0.75 | ¾ range | |
| **1.0** | full range | **identical to its `full_range`** |
| 1.5 / 2.0 | wider | the `orb` engine cannot express these |

0.5 and 1.0 reproduce the two fixed modes **trade for trade, to the cent** —
asserted in `tests/test_orb_reverse.py`. Every result produced before this
...
| file | what it is |
|---|---|
| `strategy.py` | `OrbReverseStrategy` — overrides `_stop_price` and `_open_trade`, nothing else |
| `settings.py` | `OrbReverseSettings` — the options above |
| `grid.py` | the sweep axes, including `sl_range_mult` |
| `__init__.py` | registers the engine under the name `orb_reverse` |
